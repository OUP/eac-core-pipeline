...
public class AWSV4Auth {
	...
	Logger logger = Logger.getLogger(AWSV4Auth.class);
    ...
    private String generateHex(String data) {
        ...
        try {
        	if (debug) logger.debug("payload:\n"+data);
            messageDigest = MessageDigest.getInstance("SHA-256");
            messageDigest.update(data.getBytes("UTF-8"));
            byte[] digest = messageDigest.digest();
            String hexData = String.format("%064x", new java.math.BigInteger(1, digest));
        	/*MessageDigest digest = MessageDigest.getInstance("SHA-256");
            byte[] encodedhash = digest.digest(data.getBytes(StandardCharsets.UTF_8));
            String hexData = toHexString(encodedhash);*/
            if (debug) logger.debug("hex of payload:\n"+hexData);
            return hexData;
