...
public class RBACTest extends AbstractDBTest {
	...
	@Autowired
	private AuthenticationProvider eacAuthenticationProvider;
	...
	public void setUp() throws Exception {
		...
		Permission createRegistrationDefinition = getSampleDataCreator().createPermission("CREATE_REGISTRATION_DEFINITION");
    	Permission createCustomer = getSampleDataCreator().createPermission("CREATE_CUSTOMER");
    	Permission listCustomer = getSampleDataCreator().createPermission("LIST_CUSTOMER");
    	Permission updateCustomer = getSampleDataCreator().createPermission("UPDATE_CUSTOMER");
    	Permission createActivationCodeBatch = getSampleDataCreator().createPermission("CREATE_ACTIVATION_CODE_BATCH");
    	
    	Role sysAdmin = getSampleDataCreator().createRole("SYSTEM_ADMIN");
    	Role divAdmin = getSampleDataCreator().createRole("DIVISION_ADMIN");
    	Role prodAdmin = getSampleDataCreator().createRole("PRODUCTION_ADMIN");
    	Role fieldRep = getSampleDataCreator().createRole("FIELD_REP");
	...
	@Test
	public void testSystemAdmin() throws Exception {
		Authentication authentication = authenticate(systemAdmin);
		assertEquals("Check correct number of roles", 1, ((User)SecurityContextUtils.getCurrentUser()).getRoles().size());
		assertEquals("Check correct number of roles and permissions", 7, authentication.getAuthorities().size());
	}
	
	@Test
	public void testCreateCustomer() throws Exception {
		authenticate(systemAdmin);
		Customer cust = SampleDataFactory.createCustomer() ;
		cust.setId(null);
		customerService.saveCustomer(cust, false);
		
		Customer cust1 = SampleDataFactory.createCustomer() ;
		cust1.setId(null);
		authenticate(divisonAdmin);
		customerService.saveCustomer(cust1, false);
		
		Customer cust2 = SampleDataFactory.createCustomer() ;
		cust2.setId(null);
		authenticate(productionAdmin);
		customerService.saveCustomer(cust2, false);
		
		Customer cust3 = SampleDataFactory.createCustomer() ;
		cust3.setId(null);
		authenticate(fieldRepAdmin);
		customerService.saveCustomer(cust3, false);
	}
	 ...
	 */
	@Test
	public void testCreatRegistrationDefinition() throws Exception {
		
		RegisterableProduct product = SampleDataFactory.createRegisterableProduct(12345,"product 12345", RegisterableType.SELF_REGISTERABLE);
		LicenceTemplate licenceTemplate = SampleDataFactory.createRollingLicenceTemplate();
		...
		RegistrationDefinition rd = SampleDataFactory.createProductRegistrationDefinition(product, licenceTemplate, registrationActivation, pageDefinition);
	...
	private Authentication authenticate(final AdminUser adminUser) {
		Authentication auth = eacAuthenticationProvider.authenticate(new UsernamePasswordAuthenticationToken(adminUser.getUsername(), adminUser.getPassword()));
