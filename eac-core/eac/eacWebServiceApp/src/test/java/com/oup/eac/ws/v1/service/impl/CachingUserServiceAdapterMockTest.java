...
public class CachingUserServiceAdapterMockTest extends AbstractMockTest {
    ...
    public CachingUserServiceAdapterMockTest() throws NamingException {
        super();
    }
    ...
    public void setUp() throws Exception {
        ...
        question3 = SampleDataFactory.createQuestion();
        Element element3 = SampleDataFactory.createElement(question3);
        ...
        answer3 = SampleDataFactory.createAnswer(customerWithAnswers, question3);
        ...
        setMocks(wsCustomerLookup);
        cachingUserServiceAdapter = new CachingUserServiceAdapter(wsCustomerLookup);
    }

    @Test
    public void testGetUsername() throws Exception {

        expect(wsCustomerLookup.getCustomerByWsUserId(eqSessionKey(SESSION_KEY))).andReturn(customer);

        replay(getMocks());

        UserNameResponse userNameResponse = cachingUserServiceAdapter.getUserName(SESSION_KEY);
    ...
    public void testGetUsernameCustomerNotFound() throws Exception {
        ServiceLayerException sle = new CustomerNotFoundServiceLayerException("not found");
        WebServiceValidationException ve = new WebServiceValidationException("oops", sle);
        expect(wsCustomerLookup.getCustomerByWsUserId(eqSessionKey(SESSION_KEY))).andThrow(ve);
    ...
    public void testGetRegistrationInformation() throws Exception {
        ...
        expect(wsCustomerLookup.getCustomerByWsUserId(eqSessionKey(SESSION_KEY))).andReturn(customerWithAnswers);
        
        expect(wsCustomerLookup.getCustomerWithAnswers(customerWithAnswers.getId())).andReturn(answers);

        replay(getMocks());

        RegistrationInformationResponse response = cachingUserServiceAdapter.getRegistrationInformation(SESSION_KEY);
        ...
        Assert.assertEquals(userWithAnswers.getEmailAddress(), responseUser.getEmailAddress());
        Assert.assertEquals(userWithAnswers.getFirstName(), responseUser.getFirstName());
        Assert.assertEquals(userWithAnswers.getLastName(), responseUser.getLastName());
        Assert.assertEquals(userWithAnswers.getUserName(), responseUser.getUserName());

        RegistrationInformation[] registrationInformations = response.getRegistrationInformation();

        assertNotNull(registrationInformations);
        assertEquals(3, registrationInformations.length);
        ...
        registrationInformation = getRegistrationInformationForKey(question2.getDescription(), registrationInformations);

        assertNotNull(registrationInformation);
        assertEquals(question2.getDescription(), registrationInformation.getRegistrationKey());
        assertEquals(answer2.getAnswerText(), registrationInformation.getRegistrationValue());

        registrationInformation = getRegistrationInformationForKey(question3.getDescription(), registrationInformations);

        assertNotNull(registrationInformation);
        assertEquals(question3.getDescription(), registrationInformation.getRegistrationKey());
        assertEquals(answer3.getAnswerText(), registrationInformation.getRegistrationValue());
    ...
    protected WsUserId eqUsername(final String expectedUsername) {
        IArgumentMatcher matcher = new IArgumentMatcher() {
            ...
            public boolean matches(Object arg) {
                ...
                boolean result = expectedUsername.equals(wsUserId.getUserName());
                return result;
            }

            @Override
            public void appendTo(StringBuffer out) {
                out.append("eqUsername(");
                out.append(expectedUsername);
                out.append(")");
            }
        };
    ...
    protected WsUserId eqSessionKey(final String expectedSessionKey) {
        IArgumentMatcher matcher = new IArgumentMatcher() {
            ...
            @Override
            public void appendTo(StringBuffer out) {
                out.append("eqSessionKey(");
                out.append(expectedSessionKey);
                out.append(")");
            }
        };
        EasyMock.reportMatcher(matcher);
