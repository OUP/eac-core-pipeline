...
public class UserEntitlementsAdapterMockTest/* extends AbstractMockTest */ {
	/*
	 * 
	 * public UserEntitlementsAdapterMockTest() throws NamingException { super(); }
	 * 
	 * private static final String SYSTEM_ID1 = "SYSTEM_ID1";
	 * 
	 * // system under test private UserEntitlementsAdapter sut;
	 * 
	 ...
	 * 
	 * private Customer customer; private ProductEntitlement ent1; private
	 * ProductEntitlement ent2; private ProductEntitlement ent3; private User user;
	 ...
	 * User(); this.mRegistrationService =
	 ...
	 * 
	 * this.setMocks(mCustomerLookup, mRegistrationService, mCustomerConverter,
	 * mExternalIdService, mExternalSystemLookup, mUserEntitlementsService,
	 * mEntitlementsHelperAdapter);
	 * 
	 * this.sut = new UserEntitlementsAdapterImpl( mCustomerLookup,
	 ...
	 * customer);
	 * 
	 * expect(mRegistrationService.getEntitlementsForCustomerRegistrations(customer,
	 * null,false)).andThrow(new ServiceLayerException("oops1"));
	 * 
	 * mExternalSystemLookup.validateExternalSystem(SYSTEM_ID1); expectLastCall();
	 * 
	 ...
	 * try {
	 * 
	 * @SuppressWarnings("unused") GetUserEntitlementsResponse result =
	 * sut.getUserEntitlementGroups(SYSTEM_ID1, this.wsUserId);
	 ...
	 * replay(getMocks());
	 * 
	 * GetUserEntitlementsResponse result = sut.getUserEntitlementGroups(SYSTEM_ID1,
	 * this.wsUserId); Assert.assertEquals(StatusCode.CLIENT_ERROR,
	 ...
	 * 
	 * expect(mCustomerLookup.getCustomerByWsUserId(this.wsUserId)).andReturn(
	 * customer); CustomerRegistrationsDto custRegDto = new
	 * CustomerRegistrationsDto(customer, registrations, licences);
	 * expect(mRegistrationService.getEntitlementsForCustomerRegistrations(customer,
	 * null,false)).andReturn(custRegDto);
	 * 
	 * mExternalSystemLookup.validateExternalSystem(SYSTEM_ID1); expectLastCall();
	 * 
	 ...
	 * group3); expect(mUserEntitlementsService.getUserEntitlementGroups(custRegDto,
	 * SYSTEM_ID1)).andReturn(groupList);
	 * expect(mExternalIdService.getExternalCustomerIds(customer,SYSTEM_ID1)).
	 * andReturn(extCustDto);
	 * 
	 * ProductEntitlement pe1 = new ProductEntitlement(); ProductEntitlement
	 * pe1Linked1 = new ProductEntitlement(); ProductEntitlement pe1Linked2 = new
	 * ProductEntitlement();
	 * 
	 * ProductEntitlement pe2 = new ProductEntitlement(); ProductEntitlement
	 * pe2Linked1 = new ProductEntitlement(); ProductEntitlement pe2Linked2 = new
	 * ProductEntitlement();
	 * 
	 * ProductEntitlement pe3 = new ProductEntitlement(); ProductEntitlement
	 * pe3Linked1 = new ProductEntitlement(); ProductEntitlement pe3Linked2 = new
	 * ProductEntitlement();
	 * 
	 * expect(mEntitlementsHelperAdapter.getProductEntitlement(g1Ent)).andReturn(pe1
	 * ); expect(mEntitlementsHelperAdapter.getProductEntitlement(g1LinkedEnt1)).
	 * andReturn(pe1Linked1);
	 * expect(mEntitlementsHelperAdapter.getProductEntitlement(g1LinkedEnt2)).
	 * andReturn(pe1Linked2);
	 * 
	 * expect(mEntitlementsHelperAdapter.getProductEntitlement(g2Ent)).andReturn(pe2
	 * ); expect(mEntitlementsHelperAdapter.getProductEntitlement(g2LinkedEnt1)).
	 * andReturn(pe2Linked1);
	 * expect(mEntitlementsHelperAdapter.getProductEntitlement(g2LinkedEnt2)).
	 * andReturn(pe2Linked2);
	 * 
	 * expect(mEntitlementsHelperAdapter.getProductEntitlement(g3Ent)).andReturn(pe3
	 * ); expect(mEntitlementsHelperAdapter.getProductEntitlement(g3LinkedEnt1)).
	 * andReturn(pe3Linked1);
	 * expect(mEntitlementsHelperAdapter.getProductEntitlement(g3LinkedEnt2)).
	 * andReturn(pe3Linked2);
	 * 
	 * replay(getMocks());
	 * 
	 * // CALL THE METHOD WE ARE TESTING GetUserEntitlementsResponse result =
	 * sut.getUserEntitlementGroups(SYSTEM_ID1, wsUserId); verify(getMocks());
	 * 
	 * GetUserEntitlementsResponseSequence groupSeq =
	 * result.getGetUserEntitlementsResponseSequence(); ProductEntitlementGroup[]
	 * wsGroups = groupSeq.getEntitlementGroup(); ProductEntitlementGroup wsGroup1 =
	 * wsGroups[0]; ProductEntitlementGroup wsGroup2 = wsGroups[1];
	 * ProductEntitlementGroup wsGroup3 = wsGroups[2]; checkGroups(wsGroup1, pe1,
	 * pe1Linked1, pe1Linked2); checkGroups(wsGroup2, pe2, pe2Linked1, pe2Linked2);
	 * checkGroups(wsGroup3, pe3, pe3Linked1, pe3Linked2); }
	 * 
	 * private void checkGroups(ProductEntitlementGroup wsGroup, ProductEntitlement
	 * top, ProductEntitlement... children) { Assert.assertEquals(top,
	 * wsGroup.getEntitlement()); for(int i=0;i<children.length;i++){
