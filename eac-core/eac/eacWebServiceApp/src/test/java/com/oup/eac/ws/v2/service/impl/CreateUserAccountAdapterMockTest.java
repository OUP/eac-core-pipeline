...
public class CreateUserAccountAdapterMockTest /* extends AbstractMockTest */ {
	 ...
	 * private CreateUserAccountAdapterImpl sut; private static String
	 * USERNAME_POL_REGEX = "^(?=^(?:(?!<.*?>).)*$)\\S{5,255}$"; private
	 * EmailService emailService; private VelocityEngine velocityEngine; private
	 * RegistrationService registrationService; private AnswerDao answerDao;
	 * 
	 ...
	 * ActivationCodeService activationCodeService; private
	 * RegistrationDefinitionDao registrationDefinitionDao; private
	 * LinkedRegistrationDao linkedRegistrationDao; private PageDefinitionService
	 * pageDefinitionService; private LicenceDescriptionGeneratorSource
	 * licenceDescriptionGeneratorSource; private ActivationCodeDao
	 * activationCodeDao; private RegistrationDefinitionService
	 * registrationDefinitionService; private String const_username; private
	 ...
	 * const_username; }
	 * 
	 * public CreateUserAccountAdapterMockTest() throws NamingException { super(); }
	 * 
	 ...
	 * createMock(ExternalIdDao.class); activationCodeService =
	 * createMock(ActivationCodeService.class); pageDefinitionService =
	 * createMock(PageDefinitionService.class); licenceDescriptionGeneratorSource =
	 * createMock(LicenceDescriptionGeneratorSource.class);
	 * registrationDefinitionService =
	 * createMock(RegistrationDefinitionService.class);
	 * 
	 ...
	 * 
	 * 
	 * setMocks(activationCodeService, pageDefinitionService,
	 * licenceDescriptionGeneratorSource, registrationDefinitionService,
	 * emailService, answerDao, customerDao, registrationDao, extSysDao,
	 * licenceService, externalSystemTypeDao, externalIdDao, productService,
	 * mMessageSource); registrationService = new RegistrationServiceImpl(answerDao,
	 * registrationDao, erightsFacade, emailService, velocityEngine,
	 * activationCodeService, registrationDefinitionDao, linkedRegistrationDao,
	 * mMessageSource, licenceService, pageDefinitionService,
	 * licenceDescriptionGeneratorSource, activationCodeDao, productService,
	 * registrationDefinitionService,divisionService); mCustomerService = new
	 ...
	 * UsernameValidatorImpl(USERNAME_POL_REGEX);
	 * 
	 * 
	 * 
	 * this.sut = new CreateUserAccountAdapterImpl(mMessageSource, mCustomerService,
	 * mCustomerConverter, mExternalIdService,usernameValidator); }
	 * 
	 * @Test public void testNull() { CreateUserAccountResponse result =
	 * sut.createUserAccounts(null); Assert.assertNotNull(result);
	 * Assert.assertEquals(0, result.getUserStatus().length); }
	 * 
	 * @Test public void testEmpty() { CreateUser[] empty = {};
	 ...
	 * ServiceLayerException, ErightsException { CreateUser user = getCreateUser();
	 * checkSuccess(user); }
	 * 
	 * private void checkSuccess(CreateUser userData) throws
	 * WebServiceValidationException, ServiceLayerException, ErightsException {
	 * 
	 * CreateUser[] users = { userData };
	 * 
	 * //Customer customer = new Customer(); //CustomerDto customerDto = new
	 * CustomerDto(customer); WsUserIdDto userIdDto = new WsUserIdDto();
	 * userIdDto.setUserName(userData.getCredentials().getPasswordCredential().
	 * getUserName());
	 * expect(mCustomerService.getCustomerByUsername("davidhay")).andReturn(null);
	 * expect(mCustomerConverter.convertCreateUserToCustomer(userData)).andReturn(
	 * customer);
	 * EasyMock.expect(this.usernameValidator.isValid("davidhay")).andReturn(true);
	 * EasyMock.expect(mCustomerService.validateUserAccount(userIdDto)).andReturn(
	 * customer.getId()); EasyMock.expect(mCustomerService.saveCustomer(customer,
	 * false)).andReturn(customerDto); expectLastCall();
	 * 
	 * replay(getMocks());
	 * 
	 ...
	 * ServiceLayerException, ErightsException { CreateUser userA =
	 * getCreateUser("A"+getRandomString()); addExternalId(userA,
	 * "0198608910"+const_username, "DLP", "ISBN-13"); CreateUser userB =
	 * getCreateUser("B"+const_username); CreateUser userC =
	 * getCreateUser("C"+const_username); CreateUser userD =
	 * getCreateUser("D"+const_username); CreateUser[] users = { userA, userB,
	 * userC, userD }; //String randomMsg = UUID.randomUUID().toString();
	 * 
	 * for (CreateUser userData : users) { Customer customer = new Customer();
	 * expect(mCustomerService.getCustomerByUsername(getUserName(userData))).
	 * andReturn(null);
	 * expect(mCustomerConverter.convertCreateUserToCustomer(userData)).andReturn(
	 * customer); mCustomerService.saveCustomer(customer, false);
	 ...
	 * ServiceLayerException, ErightsException { CreateUser userA =
	 * getCreateUser("A"+setConstUserName()); CreateUser userB =
	 * getCreateUser("B"+const_username);
	 * 
	 * CreateUser userC = getCreateUser("C"+const_username); CreateUser userD =
	 * getCreateUser("D"+const_username);
	 * 
	 ...
	 * null);
	 * expect(mCustomerConverter.convertCreateUserToCustomer(userA)).andReturn(
	 * customerA); mCustomerService.saveCustomer(customerA, false);
	 * expectLastCall();
	 * 
	 * //B
	 * expect(mCustomerService.getCustomerByUsername(getUserName(userB))).andReturn(
	 * null); mCustomerService.saveCustomer(customerA, false); expectLastCall(); //
	 * C Customer customerC = new Customer();
	 * expect(mCustomerService.getCustomerByUsername(getUserName(userC))).andReturn(
	 * null);
	 * expect(mCustomerConverter.convertCreateUserToCustomer(userC)).andReturn(
	 * customerC); mCustomerService.saveCustomer(customerC, false);
	 * expectLastCall();
	 * 
	 * //D
	 ...
	 * //verify(getMocks()); }
	 * 
	 * private CreateUser getCreateUser(String username) { CreateUser result =
	 * getCreateUser();
	 * result.getCredentials().getPasswordCredential().setUserName(username); return
	 * result; }
	 * 
	 * private CreateUser addExternalId(CreateUser createUser, String id, String
	 * systemId, String typeId) { ExternalIdentifier externalIdentifier = new
	 * ExternalIdentifier(); externalIdentifier.setId(id);
	 * externalIdentifier.setSystemId(systemId);
	 * externalIdentifier.setTypeId(typeId);
	 * createUser.addExternal(externalIdentifier); return createUser; }
	 * 
	 * @Test public void testAlreadyExists() throws WebServiceValidationException,
	 * ServiceLayerException, ErightsException { CreateUser user = getCreateUser();
	 * CreateUser[] users = { user }; //Customer customer = new Customer();
	 * //expect(mCustomerService.getCustomerByUsername("davidhay")).andReturn(
	 * customer);
	 * //EasyMock.expect(this.usernameValidator.isValid("davidhay")).andReturn(true)
	 * ;
	 * 
	 * //replay(getMocks());
	 * 
	 * CreateUserAccountResponse result = sut.createUserAccounts(users); result =
	 * sut.createUserAccounts(users);
	 * 
	 * checkClientError(user, result, "The username is already taken."); }
	 * 
	 * @Test public void testValidBlankFirstname() throws
	 * WebServiceValidationException, ServiceLayerException, ErightsException {
	 * CreateUser user = getCreateUser(); user.setFirstName(""); checkSuccess(user);
	 * }
	 * 
	 * @Test public void testValidBlankLastname() throws
	 * WebServiceValidationException, ServiceLayerException, ErightsException {
	 * CreateUser user = getCreateUser(); user.setLastName(""); checkSuccess(user);
	 * }
	 * 
	 * @Test public void testInvalidUsername() throws WebServiceValidationException,
	 * ServiceLayerException, ErightsException { CreateUser user =
	 * getCreateUser(""); checkInvalid(user, "The username cannot be blank.",false);
	 * }
	 * 
	 * @Test public void testInvalidLocaleLanguage() throws
	 * WebServiceValidationException, ServiceLayerException, ErightsException {
	 * CreateUser user = getCreateUser(); LocaleType lt = new LocaleType();
	 * lt.setLanguage("abc"+setConstUserName()); user.setLocale(lt);
	 * //EasyMock.expect(this.usernameValidator.isValid("davidhay")).andReturn(true)
	 ...
	 * WebServiceValidationException, ServiceLayerException, ErightsException {
	 * CreateUser user = getCreateUser(); LocaleType lt = new LocaleType();
	 * lt.setLanguage("en"); user.setLocale(lt); checkSuccess(user); }
	 * 
	 * @Test public void testInvalidLocaleCountry1() throws
	 * WebServiceValidationException, ServiceLayerException, ErightsException {
	 * CreateUser user = getCreateUser(); LocaleType lt = new LocaleType();
	 * lt.setLanguage("en"); lt.setCountry("SCO"); user.setLocale(lt);
	 * //EasyMock.expect(this.usernameValidator.isValid("davidhay")).andReturn(true)
	 ...
	 * 
	 * private CreateUser getCreateUser() { CreateUser user = new CreateUser();
	 * 
	 ...
	 * return user; }
	 * 
	 * private void checkInvalid(CreateUser user, String expectedErrorMessage)
	 * throws ErightsException { checkInvalid(user,expectedErrorMessage,true); }
	 * 
	 * private void checkInvalid(CreateUser user, String expectedErrorMessage,
	 * boolean checkCustomerExists) throws ErightsException { CreateUser[] users = {
	 * user };
	 * 
	 ...
	 * 
	 * private void checkClientError(CreateUser user, CreateUserAccountResponse
	 * result, String expectedErrorMessage) { Assert.assertNotNull(result);
	 * Assert.assertEquals(1, result.getUserStatus().length); UserStatusType us1 =
	 * result.getUserStatus(0);
	 * 
	 ...
	 * 
	 * private String getUserName(CreateUser user) { return
	 ...
	 * 
	 * CreateUser user1 = getCreateUser(); CreateUser user2 = getCreateUser();
	 * CreateUser[] users = { user1, user2 }; Customer customer1 = new Customer();
	 ...
	 * null);
	 * 
	 * expect(mCustomerConverter.convertCreateUserToCustomer(user1)).andReturn(
	 * customer1);
	 * expect(mCustomerConverter.convertCreateUserToCustomer(user2)).andReturn(
	 * customer2);
	 * 
	 ...
	 * expectLastCall().andThrow(new ServiceLayerValidationException(randomMsg));
	 * 
	 * replay(getMocks()); CreateUserAccountResponse result =
	 * this.sut.createUserAccounts(users); //verify(getMocks()); UserStatusType[]
	 * status = result.getUserStatus(); Assert.assertEquals(2,status.length);
	 * UserStatusType status1 = status[0]; UserStatusType status2 = status[1];
	 * 
	 * Assert.assertEquals(randomMsg,status1.getErrorStatus().getStatusReason());
	 * Assert.assertEquals(StatusCode.CLIENT_ERROR,status1.getErrorStatus().
	 * getStatusCode());
	 * 
	 ...
	 * EasyMock.expect(this.usernameValidator.isValid(user2.getCredentials().
	 * getPasswordCredential().getUserName())).andReturn(true);
	 * EasyMock.expect(mCustomerService.saveCustomer(customer1,
	 * false)).andReturn(customerDto); expectLastCall(); replay(getMocks());
	 * CreateUserAccountResponse result = this.sut.createUserAccounts(users);
	 * //verify(getMocks()); UserStatusType[] status = result.getUserStatus();
	 * Assert.assertEquals(2,status.length); UserStatusType status1 = status[0];
	 * UserStatusType status2 = status[1];
	 * 
	 * Assert.assertEquals(randomMsg,status1.getErrorStatus().getStatusReason());
	 * Assert.assertEquals(StatusCode.SERVER_ERROR,status1.getErrorStatus().
	 * getStatusCode());
	 * 
	 * 
	 * }
	 ...
	 * 
	 * 
	 * String uniqueUserName = groupCommonUtil.getRandomString(); OupUserWS
	 * oupUserWS = new OupUserWS(); oupUserWS.setConcurrency("3");
	 * oupUserWS.setEmailAddress(uniqueUserName + "_Test@TestHost.com");
	 * oupUserWS.setSuspended("false"); OupAuthProfileWS oupAuthProfileWS = new
	 * OupAuthProfileWS(); oupAuthProfileWS.setMatchMode(MatchMODE.ANY.toString());
	 * OupCredentialWS oupCredentialWS = new OupCredentialWS();
	 * OupCredentialLoginPasswordWS oupCredentialDataWS = new
	 * OupCredentialLoginPasswordWS();
	 ...
	 * oupUserWS.setPrivacyPolicyAccepted(String.valueOf(Boolean.TRUE)); return
	 * oupUserWS;
	 * 
	 * 
	 * }
