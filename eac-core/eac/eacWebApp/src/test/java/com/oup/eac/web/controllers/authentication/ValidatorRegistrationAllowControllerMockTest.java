...
public class ValidatorRegistrationAllowControllerMockTest /* extends AbstractMockTest */ {
	 ...
	 * RegisterableProduct product; private TestingAppender testingAppender;
	 * 
	 * 
	 ...
	 * ValidatorRegistrationAllowController(); this.testingAppender = new
	 * TestingAppender();
	 * 
	 * }
	 * 
	 ...
	 * false; checkAllowLink(activated, TEST_URL); Assert.assertEquals(0,
	 * testingAppender.getMessages().size()); }
	 * 
	 ...
	 * map1.put("registration", registration); map1.put("username",
	 * TEST_CUSTOMER_USERNAME); map1.put("product", product.getProductName());
	 * 
	 * ModelAndView mav = sut.handleRequest(request, response);
	 * 
	 * Assert.assertEquals("validatorLicenceAllowed", mav.getViewName());
	 * Map<String,Object> model = mav.getModel(); Assert.assertEquals(2,
	 * model.size()); Assert.assertEquals(TEST_CUSTOMER_USERNAME,
	 * model.get("username")); Assert.assertEquals(TEST_PRODUCT_NAME,
	 * model.get("product")); Assert.assertEquals(TEST_PRODUCT_HOME,
	 * model.get("productHome")); Assert.assertEquals(TEST_PRODUCT_EMAIL,
	 * model.get("email")); //Assert.assertEquals(TEST_URL,
