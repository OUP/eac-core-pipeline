...
public class AuthenticationWorkFlowTest extends AbstractDbMessageTest {
	...
	@Test
	public void testGetErightsLicenceDecisions() {
		Set<ErightsLicenceDecision> decisions = AuthenticationWorkFlow.getErightsLicenceDecisions(request);
		assertEquals(1, decisions.size());
		ErightsLicenceDecision erightsLicenceDecision = decisions.iterator().next();
		assertNotNull(erightsLicenceDecision);
		assertEquals(ErightsLicenceDecision.DENY_CONCURRENCY, erightsLicenceDecision);
	}
	
	@Test
	public void testGetErightsLicenceDecisionsMultiple() {
		request.setParameter("licenseDenyTypes", "-510,5");
		Set<ErightsLicenceDecision> decisions = AuthenticationWorkFlow.getErightsLicenceDecisions(request);
		assertEquals(2, decisions.size());
		int countDenyConcurreny = 0;
		int countDenyAccept= 0;
		for(ErightsLicenceDecision erightsLicenceDecision : decisions) {
			...
			if(erightsLicenceDecision == ErightsLicenceDecision.DECISION_ACCEPT) {
				countDenyAccept++;
			}
		}
		assertEquals(countDenyConcurreny, 1);
		assertEquals(countDenyAccept, 1);
	}
	
	@Test
	public void testGetErightsLicenceDecisionsNoReasons() {
	...
	@Test
	public void testIsConcurrencyExceededNoReasons() {
		request.setParameter("licenseDenyTypes", "");
		assertFalse(AuthenticationWorkFlow.isConcurrencyExceeded(request));
	}
	
	@Test
	public void testIsConcurrencyExceeded() {
		request.setParameter("licenseDenyTypes", "-510,5");
