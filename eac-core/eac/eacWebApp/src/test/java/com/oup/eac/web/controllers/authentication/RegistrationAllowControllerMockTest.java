...
public class RegistrationAllowControllerMockTest extends AbstractMockTest {
    ...
    public void setup() {
        ...
        sut = new RegistrationAllowController(registrationService);
    }
    ...
    @Test
    public void testNotActivatedBlankUrl() throws Exception {
        boolean activated = false;
        checkAllowLink(activated, "");
    }
    ...
    @Test
    public void testActivatedBlankUrl() throws Exception {
        boolean activated = true;
        checkAllowLink(activated, "");
    }
    ...
    private void checkAllowLink(boolean activated, String originalUrl) throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.setParameter("token", "tokenValue");
        MockHttpServletResponse response = new MockHttpServletResponse();
        ...
        EasyMock.expect(this.registrationService.updateAllowRegistrationFromToken("tokenValue", true)).andReturn(map1);
        
        replayMocks();
        ModelAndView mav = sut.handleRequest(request, response);
        RedirectView redirect = (RedirectView)mav.getView();
        if(StringUtils.isBlank(originalUrl)){
            Assert.assertEquals(null, redirect.getUrl());
        }else{
            Assert.assertEquals(originalUrl, redirect.getUrl());
        }
