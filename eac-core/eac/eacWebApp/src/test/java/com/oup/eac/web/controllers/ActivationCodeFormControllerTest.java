...
public class ActivationCodeFormControllerTest extends AbstractDbMessageTest {
	...
	private static final String SPRING_VALIDATION_BINDING_RESULT = "org.springframework.validation.BindingResult.";
	
	private static final Logger LOGGER = Logger.getLogger(ActivationCodeFormControllerTest.class);
    ...
    public final void setUp() throws Exception {
    	...
    	RegistrationActivation registrationActivation = getSampleDataCreator().createInstantRegistrationActivation();
    	Division division = getSampleDataCreator().createDivision("MALAYSIA");
    	RegisterableProduct product = SampleDataFactory.createRegisterableProduct(1,"Product 1", RegisterableType.SELF_REGISTERABLE);
    	...
    	ActivationCodeRegistrationDefinition acrd = getSampleDataCreator().createActivationCodeRegistrationDefinition(product, licenceTemplate, registrationActivation, null);
        ActivationCodeBatch activationCodeBatch = getSampleDataCreator().createActivationCodeBatch(licenceTemplate, ActivationCodeFormat.EAC_NUMERIC, acrd, null, null);
        activationCode1 = getSampleDataCreator().createActivationCode(activationCodeBatch, new EacNumericActivationCode());
        customer = getSampleDataCreator().createCustomer();
        SessionHelper.setCustomer(httpServletRequest, customer);
    	loadAllDataSets();
    } 
    
    @Ignore
    @Test
    public void testValidActivationCode() throws Exception {
        ...
        ModelAndView modelAndView = activationCodeFormController.handleRequest(httpServletRequest, httpServletResponse);
        assertFalse(hasErrors(modelAndView, "activationCode"));
        assertEquals("http://localhost/product1.html", getView(modelAndView));
    }    
    
    @Ignore
    @Test
    public void testInValidActivationCode() throws Exception {
    	httpServletRequest.setMethod("POST");
    	SessionHelper.setForwardUrl(httpSession, "http://localhost/product1.html");
    	httpServletRequest.setParameter("code", "1234");
    	
        ModelAndView modelAndView = activationCodeFormController.handleRequest(httpServletRequest, httpServletResponse);
        assertTrue(hasErrors(modelAndView, "activationCode"));
    ...
    private final boolean hasErrors(final ModelAndView modelAndView, final String key) {
    	BeanPropertyBindingResult result = (BeanPropertyBindingResult)modelAndView.getModelMap().get(SPRING_VALIDATION_BINDING_RESULT + key);
