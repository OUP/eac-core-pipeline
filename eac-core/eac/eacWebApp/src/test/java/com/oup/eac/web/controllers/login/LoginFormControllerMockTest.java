...
public class LoginFormControllerMockTest extends AbstractMockTest {
   ...
   // private static final String ERROR_VIEW = "error";
    ...
    public LoginFormControllerMockTest() throws NamingException {
        super();
    }
    ...
    public void setup() {
        ...
        localeResolver = EasyMock.createMock(LocaleResolver.class);
        domainSkinResolverService = EasyMock.createMock(DomainSkinResolverService.class);
        whiteListUrlService = EasyMock.createMock(WhiteListUrlService.class);
        productService = EasyMock.createMock(ProductService.class);
        whiteListdao =EasyMock.createMock(WhiteListUrlDao.class);
        controller = new LoginFormController(customerService, domainSkinResolverService, productService, whiteListUrlService);
        controller.setFormView(FORM_VIEW);
        controller.setCommandClass(LoginDto.class);
        request.setAttribute(DispatcherServlet.LOCALE_RESOLVER_ATTRIBUTE, localeResolver);
        request.setParameter(URL_PARAM, URL);
        unique = UUID.randomUUID().toString().replace("-", "");
        setMocks(customerService, localeResolver, whiteListUrlService, whiteListdao);
       
        command = new LoginDto();
        command.setUsername(USERNAME);
        command.setPassword(PASSWORD);
        errors = new BindException(command, "loginDto");
    ...
    public void testHappyPath1() throws Exception {
        ...
        EasyMock.expect(customerService.getCustomerByUsernameAndPassword(USERNAME, PASSWORD, true, true)).andReturn(customerSessionDto);
    ...
    public void testServiceLayerException() throws Exception {
        ...
        EasyMock.expect(customerService.getCustomerByUsernameAndPassword(USERNAME, PASSWORD, true, true)).andThrow(new ServiceLayerException());
    ...
    @Test
    public void testDirectLoginUrlNotPassed() throws Exception {
    ...
    @Test
    public void testDirectLoginUrlPassed() throws Exception {
    ...
    public void testPrimaryDomainSessionAvailableButInvalid() throws Exception {
    	...
    	EasyMock.expect(whiteListUrlService.getUrls()).andStubReturn(urlList);
    	EasyMock.expect(customerService.getCustomerFromSession(session)).andReturn(null);
    ...
    public void testPrimaryDomainSessionAvailableAndValid() throws Exception {
    	...
    	request.setCookies(CookieHelper.createErightsCookie(session));
    	EasyMock.expect(whiteListUrlService.getUrls()).andStubReturn(urlList);
    	EasyMock.expect(customerService.getCustomerFromSession(session)).andReturn(customer);
