...
public class LicenceAcceptedVelocityTest {
    ...
    private Resource expected1 = new ClassPathResource("com/oup/eac/service/impl/licenceAcceptedExpected.txt");
    private Resource expected2 = new ClassPathResource("com/oup/eac/service/impl/licenceAcceptedExpected2.txt");
    ...
    public void testTemplate1() throws Exception {
        ...
        Template t = ve.getTemplate("com/oup/eac/service/velocity/licenceAccepted.vm");
        ...
        ctx.put("licenceDescription", "Start 1 Nov 2012 End 31 Dec 2013");
        ctx.put("email", "product.admin@mailinator.com");
		ctx.put("productdivision", "NON-EMAL");
        ...
        int maxSize = Math.max(result.length(),expected.length());
        for(int i = 0;i<maxSize;i++){
            char c1 = '#';
            char c2 = '#';
            try{
                c1 = expected.charAt(i);
            }catch(Exception ex){
                c1 ='#';
            }
            try{
            ...
            System.out.printf("%4d %c(%d) %c(%d)%n",i,c1,(int)c1, c2, (int)c2);
        }
    ...
    public void testTemplate2() throws Exception {
        ...
        ctx.put("productname", "ProductXYZ");
        ctx.put("licenceDescription", "Start 1 Nov 2012 End 31 Dec 2013");
		ctx.put("productdivision", "NONELT");
        StringWriter writer = new StringWriter();
        t.merge(ctx, writer);
    ...
    public String getExpected(Resource expected) {
        try {
            InputStream is = expected.getInputStream();
