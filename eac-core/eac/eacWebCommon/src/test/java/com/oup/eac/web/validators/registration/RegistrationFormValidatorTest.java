...
public class RegistrationFormValidatorTest extends AbstractValidatorTest {
	...
	public RegistrationFormValidatorTest() throws NamingException {
		super();
	}
	...
	@Test
	public void testValidateNoAnswer() throws Exception {
		...
		element.setTitleText("title text");
		Field field = SampleDataFactory.createField(component, element, 1);
		...
		errors.rejectValue(EasyMock.eq("answers['" + question.getId() + "']"), EasyMock.eq("error.not-specified"), 
				eqLabels(new Object[] { field.getElement().getTitleText() }), EasyMock.eq(field.getElement().getTitleText() + " is required."));
		replayMocks();
	...
	@Test
	public void testValidateWithInvalidAnswer() throws Exception {
		Component component = SampleDataFactory.createComponent("label.key");
		Question question = SampleDataFactory.createQuestion();
		Element element = SampleDataFactory.createElement(question);
		SampleDataFactory.createTextField(element);
		element.setTitleText("title text");
		element.setRegularExpression("^[a-z]+$");
		Field field = SampleDataFactory.createField(component, element, 1);
		...
		errors.rejectValue(EasyMock.eq("answers['" + question.getId() + "']"), EasyMock.eq("error.regularexpression"), 
									eqLabels(new Object[] { field.getElement().getTitleText() }), 
	...
	@Test
	public void testValidateWithValidAnswer() throws Exception {
		Component component = SampleDataFactory.createComponent("label.key");
		Question question = SampleDataFactory.createQuestion();
		Element element = SampleDataFactory.createElement(question);
		SampleDataFactory.createTextField(element);
