...
/**
 * 
 * NOTE. This duplicate class exists so that we can override the definition of the 'key' column.
 * For the default Message class (in the eacDomain project) we have to back-quote the 'key'
 * column so that it doesn't break MSSQL Server. However, for the tests, HSQLDB won't work
 * if the 'key' column is back-quoted, so this class overrides the default Message without
 * a back-quoted 'key'.
 *
 */
